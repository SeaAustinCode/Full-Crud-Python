...
class User:
    def __init__( self , data ):
        ...
        self.full_name = self.first_name +" " + self.last_name # this is an option for how to combine bo
    ...
    # @classmethod ---- LEFT JOIN -- All recipes with its one user displaying a dojo name for all ninjas 
    # def get_one_dojo_and_its_ninjas(cls, data):
    #     query = "SELECT * FROM dojos LEFT JOIN ninjas ON dojos.id = ninjas.dojo_id WHERE dojos.id = %(id)s;" #Select all from (table 1) left join (table 2) on (table 1 .id) = (table2.foreign key) where (table 1.id)
    #     result = connectToMySQL(DATABASE).query_db(query,data)
